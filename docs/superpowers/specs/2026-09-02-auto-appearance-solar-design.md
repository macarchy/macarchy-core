...
- `macarchy-auto-appearance` (bash, `style/`) bascule Apple Glass ↔ Apple
...
- `macarchy-dynamic-wallpaper` contient le calcul NOAA de lever/coucher et lit
...
Trois choses, toutes dans `macarchy-core` :

1. **`macarchy-sun`** — un outil qui imprime lever et coucher du jour pour la
   position partagée.
2. **`macarchy-auto-appearance` en mode solaire** — il demande la fenêtre à
   `macarchy-sun` au lieu de la lire dans son conf.
...
qu'il consomme `macarchy-sun` (il garde sa copie du calcul).
...
- Calcule lever et coucher du **jour civil local** demandé (défaut : aujourd'hui)
  avec l'équation NOAA déjà en service dans `macarchy-dynamic-wallpaper`
...
- `MODE=solar` : `from`/`until` viennent de `macarchy-sun`. Si `state` est
  `up`, on veut le thème clair ; `down`, le sombre. Si `macarchy-sun` échoue
...
- Nouvelle sous-commande `status`, une ligne parsable pour le Control Center :
  ...
  `enabled` reflète `systemctl --user is-enabled macarchy-auto-appearance.timer`.
  En mode `schedule`, `sunrise`/`sunset` sont remplacés par `from`/`until`.

Le timer passe de `*:0/15` à `*:0/5`, aligné sur `macarchy-dynamic-wallpaper.timer`,
...
- Choisir Auto : `systemctl --user enable --now macarchy-auto-appearance.timer`.
  ...
  `macarchy-auto-appearance` directement. Le thème se corrige donc tout de
  suite.
...
- **Phrase d'état** (caption, 55 %) :
  ...
  - auto, `macarchy-sun` en échec : « Position inconnue — appuie sur Détecter. »
...
Un script, pas du QML : **`macarchy-locate`** (bash, `style/`), pour que la
...
- Succès : réécrit `latitude`/`longitude` dans `dynamic-wallpaper.json` avec
  ...
  `macarchy-auto-appearance` pour que le thème suive la nouvelle position.
...
Première utilisation : je lance `macarchy-locate` moi-même pour remplacer Paris
...
- `macarchy-auto-appearance status` remplace la lecture directe de `theme.name`
...
- `tests/test_macarchy_sun.py` — unittest, importe le script par `importlib`.
- `tests/test_auto_appearance.sh` — le script bash sous un `PATH` qui contient
  un faux `macarchy-sun`, un faux `omarchy` et un faux `systemctl`, avec l'heure
  ...
  échec de `macarchy-sun`, thème tiers.

Modifiés :

- `style/macarchy-auto-appearance` — `MODE`, `status`, échec explicite.
- `systemd/macarchy-auto-appearance.timer` — cadence 5 min.
...
- `agents/skills/omarchy-asahi/SKILL.md` — `dynamic-wallpaper.json` est aussi
  la position du thème ; `macarchy-locate` la corrige.
...
- **`systemctl enable` depuis quickshell.** `Quickshell.execDetached` hérite
  ...
  Vérifié par le fait que `macarchy-als daemon` est déjà lancé de la même façon.
...
4. `macarchy-sun` imprime des heures plausibles pour la Belgique début
...
6. Choisir Auto → `systemctl --user is-enabled macarchy-auto-appearance.timer`
