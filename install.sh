#!/bin/bash
# Install the macarchy-core suite: scripts into ~/.local/bin, the theme hook,
...
install -m755 hooks/macarchy-dock-theme "$HOME/.config/omarchy/hooks/theme-set.d/"
install -m755 hooks/dynamic-wallpaper "$HOME/.config/omarchy/hooks/theme-set.d/"
install -m755 hooks/macarchy-bar-contrast "$HOME/.config/omarchy/hooks/theme-set.d/"
...
install -m755 hooks/aquarium/macarchy-bar-contrast "$HOME/.config/omarchy-aquarium/hooks/"
...
install -m644 systemd/macarchy-bar-contrast.{service,timer} "$HOME/.config/systemd/user/"
systemctl --user daemon-reload
if (( first_install )); then
    systemctl --user enable --now macarchy-auto-appearance.timer
fi
systemctl --user enable --now macarchy-bar-contrast.timer
...
-- macarchy-keys: Cmd behaves like macOS (see github.com/macarchy/macarchy-core)
